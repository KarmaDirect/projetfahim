...
const VIDEO_DURATION = 156; // 5.2s at 30fps

export const Scene3b2_YouTubeShowcase: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // ── Badge (Beat 0.5) ──
  const badgeSpring = spring({
    frame: frame - Math.round(BEAT * 0.5),
    fps,
    config: { damping: 8, stiffness: 120, mass: 0.4 },
  });

  // ── Title (Beat 0-2) — slides from left ──
  ...
  // ── Player card slides in from RIGHT (Beat 1.5-3) ──
  ...
  const driftX = hasLanded ? Math.sin(frame * 0.012) * 3 : 0;
  const driftY = hasLanded ? Math.cos(frame * 0.016) * 2 : 0;

  // ── Click at Beat 4 ──
  const clickFrame = BEAT * 4;
  ...
  const exitProgress = interpolate(
    frame,
    [videoEndFrame + 5, videoEndFrame + 35],
    [0, 1],
  ...
  // ── Cursor animation — comes from bottom-right ──
  const cursorAppear = Math.round(BEAT * 2.8);
  ...
  // Slight hover wobble near the end
  ...
  // ── Background ──
  const orbX = 50 + Math.sin(frame * 0.007) * 12;
  const orbY = 45 + Math.cos(frame * 0.009) * 8;
    ...
    <AbsoluteFill style={{ backgroundColor: BG, overflow: "hidden" }}>
      ...
      >
        ...
        >
          ...
          >
            Ce qu'on fait pour les créateurs
          </div>
        </div>

        {/* Video player card — slides from right with rotation */}
        <div
          style={{
            marginTop: 30,
        ...
        >
          ...
          >
            ...
            >
              FahimAE — Showreel Créateurs 2024
            </span>
          </div>
          ...
          >
            {/* Placeholder gradient before video plays */}
            {!hasClicked && (
              <div
                style={{
                  position: "absolute",
            ...
            {/* Video — Sequence handles start time & duration */}
            <Sequence from={clickFrame} durationInFrames={VIDEO_DURATION} layout="none">
              <OffthreadVideo
                src={staticFile("realisation_pouryoutubeurs.mp4")}
                style={{
                  position: "absolute",
                  inset: 0,
                  width: "100%",
            ...
            {hasClicked && frame < videoEndFrame + 35 && (
              <div
                style={{
                  position: "absolute",
                  bottom: 0,
      ...
      {/* Animated cursor — bezier curve from bottom-right */}
      <div
        style={{
          position: "absolute",
